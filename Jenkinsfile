...
def getRouteHostname = { String routeName, String projectName ->
  sh "oc get route parksmap -n ${projectName} -o jsonpath='{ .spec.host }' > apphost"
  ...
  node ("maven") {
    ...
    if (isPR) {
      stage ('Create PR Project') {
        ...
        project = uniqueName("parksmap-web-")
        sh "oc new-project ${project}"
        projectCreated=true
        sh "oc create serviceaccount jenkins -n ${project}"
        sh "oc policy add-role-to-user edit -z jenkins -n ${project}"
        sh "oc policy add-role-to-group edit system:authenticated -n ${project}"
      }
    }
